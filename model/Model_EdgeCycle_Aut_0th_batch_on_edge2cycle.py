...
class Edge_cycle(torch.nn.Module):
    def __init__(self, rep_dim, num_channels, cycle_sizes) -> None:
        super().__init__()
        self.cycle_mlp = get_mlp((1 + 4 + num_channels) * rep_dim,rep_dim * _inner_mlp_mult,rep_dim,3)
        ...
        self.cycle_autobahns =ModuleList([p.Autobahn(2 * rep_dim,rep_dim * num_channels,cycle) 
    ...
    def forward(self,edge_rep,cycle_rep,G,data):
        ...
        edge2cycle_linmap = p.batched_subgraphlayer1b.cat(*edge2cycles_linmap) #nc * 4
...
class Model_EdgeCycle_Aut_0th_batch_on_edge2cycle(torch.nn.Module):
    def __init__(self, rep_dim: int, dense_dim: int, out_dim: int, num_layers: int, dropout,
        ...
        print("Running: Model_EdgeCycle_Aut_0th_batch_on_edge2cycle\n\n")
        super().__init__()
