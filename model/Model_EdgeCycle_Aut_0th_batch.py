...
class Edge_cycle(torch.nn.Module):
    def __init__(self, rep_dim, num_channels, cycle_sizes, gather_overlap=1) -> None:
        ...
        # self.cycle_mlp_2 = get_mlp(2 * rep_dim,rep_dim * _inner_mlp_mult,rep_dim,2)
        # Sequential(
    ...
    def forward(self,edge_rep,cycles_rep,G,data,cycles_rep_size0):
        edge2cycles = [p.batched_subgraphlayer1b.gather_from_ptensors(edge_rep,G,cycle,min_overlaps=self.gather_overlap) 
                       for cycle in self.cycles]
        cycles_new = [edge2cycle + cycle_rep for cycle_rep,edge2cycle in zip(cycles_rep,edge2cycles)] #nc * 2
        ...
        cycles_out = [torch.cat([cycle_linmap.torch(),cycle_aut.torch()],dim=-1)
                      for cycle_linmap, cycle_aut in zip(cycles_linmap,cycles_aut)]
        cycle_out = self.cycle_mlp(torch.cat(cycles_out,dim=0))  #nc * (num_channels + 2) -> nc
