...
class EdgeCycleAutobahnOnLinmap(torch.nn.Module):
    def __init__(self,
                 eps: float = 0.0,
                 train_eps: bool = True,
                 ...
                 bn_eps=1e-5,
                 bn_momentum=0.1):
        super(EdgeCycleAutobahnOnLinmap, self).__init__()
        print("initializing a model with autobahn")
        print("initializing a model with autobahn on cycle linmap as well")
        ...
        self.eps_cycle_3 = torch.nn.Parameter(torch.Tensor([[0.]]),
                                            requires_grad=True)
        ...
        if subgraphs is not None:
            ...
            self.included_cycles += [3] * len(subgraphs) # 3 to prevent gather from overlapping with the edge
        ...
        self.linmap_autobahn = torch.nn.ModuleList([p.Autobahn(edge_hidden_dim * 2, autobahn_channels, cycle) for cycle in self.cycles])


        self.cycle_autobahn_mlp = get_mlp_invertible(autobahn_channels * 2, cycle_dense_dim, cycle_hidden_dim * 2, num_layers=2, activation="ReLU", inverse=False)

        self.cycle_autobahn_linmap_mlp = get_mlp_invertible(autobahn_channels, cycle_dense_dim, cycle_hidden_dim * 2, num_layers=2, activation="ReLU", inverse=False)
    ...
    def forward(self, data, edge_attr, G, cycle_attr=None):
        ...
        cycle_linmap_lst = [p.batched_subgraphlayer1b.gather_from_ptensors(cycle_attr, G, cycle, min_overlaps=size) for size, cycle in zip(self.included_cycles, self.cycles)]

        cycle_linmap_autobahn_lst = [
            autobahn(p.batched_subgraphlayer1b.gather_from_ptensors(cycle_attr, G, cycle, min_overlaps=size)) for autobahn, size, cycle in zip(self.linmap_autobahn, self.included_cycles, self.cycles)]

        cycle_linmap = p.batched_subgraphlayer1b.cat(*cycle_linmap_lst)

        cycle_linmap_autobahn = self.cycle_autobahn_linmap_mlp(p.batched_subgraphlayer1b.cat(*cycle_linmap_autobahn_lst).torch())
        ...
        cycle_out = self.cycle_mlp_1((1 + self.eps_cycle_1) * cycle_linmap.torch() + (1 + self.eps_cycle_2) * lift_aggr + (1 + self.eps_cycle_3) * edge2cycle_autobahn + cycle_linmap_autobahn)
