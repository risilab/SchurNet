...
class Edge_cycle_aut(torch.nn.Module):
...
class Edge_node(torch.nn.Module):
    ...
    def forward(self,node_rep, edge_rep,G,data):
        ...
        # print("size of graph: ",G.size())
        print("node_rep: ",node_rep.torch().shape)
        print("edge2node: ",edge2node.torch().shape)
...
class ConvLayer(torch.nn.Module):
    def __init__(self, rep_dim: int,num_channels, dropout,cycle_sizes,use_branched_5_cycles, gather_overlap) -> None:
        ...
        self.edge_cycle = Edge_cycle_aut(rep_dim,num_channels,cycle_sizes,gather_overlap)
...
class ablation_Model_Aut_on_cycle_only_small(torch.nn.Module):
    def __init__(self, rep_dim: int, dense_dim: int, out_dim: int, num_layers: int, dropout,
        ...
        print("Running: ablation_Model_Aut_on_cycle_only_small\n\n")
        super().__init__()
