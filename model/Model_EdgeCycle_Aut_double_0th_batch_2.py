...
class Edge_cycle(torch.nn.Module):
    def __init__(self, rep_dim, num_channels, cycle_sizes, gather_overlap=1) -> None:
        super().__init__()
        self.cycle_mlp_2 = get_mlp((num_channels + 2) * rep_dim,rep_dim * _inner_mlp_mult,rep_dim,2)
        ...
        self.cycle_autobahns_1 =ModuleList([p.Autobahn(3 * rep_dim,rep_dim,cycle) 
                                          for cycle in self.cycles])
        self.cycle_autobahns_2 =ModuleList([p.Autobahn(rep_dim,rep_dim * num_channels,cycle) 
                                          for cycle in self.cycles])
    ...
    def forward(self,edge_rep,cycles_rep,G,data,cycles_rep_size0):
        ...
        cycles_new = [p.batched_subgraphlayer1b.cat_channels(cycle_rep,edge2cycle)
                      for cycle_rep,edge2cycle in zip(cycles_rep,edge2cycles)] #nc * 3
        cycles_aut = [cycle_autobahn(cycle_new).relu(0)
                       for cycle_autobahn, cycle_new in zip(self.cycle_autobahns_1,cycles_new)] #nc 
        cycles_out = [cycle_autobahn(cycle_aut).relu(0)
                       for cycle_autobahn, cycle_aut in zip(self.cycle_autobahns_2,cycles_aut)] #nc * num_channels
        cycles_out2 = [p.batched_subgraphlayer1b.gather_from_ptensors(cycle_aut,G,cycle,min_overlaps=size).relu(0)
                       for cycle_aut, cycle, size in zip(cycles_aut,self.cycles,self.cycle_sizes)] #nc * 2, cycle_aut linmap
        
        cycles_out = [torch.cat([cycle_out.torch(),cycle_out2.torch()],dim=-1) for cycle_out,cycle_out2 in zip(cycles_out,cycles_out2)]
        cycle_out = self.cycle_mlp_2(torch.cat(cycles_out,dim=0))  #nc * (num_channels + 2) -> nc
        cycles_out = torch.split(cycle_out,cycles_rep_size0,dim=0)
...
class Model_EdgeCycle_Aut_double_0th_batch_2(torch.nn.Module):
    def __init__(self, rep_dim: int, dense_dim: int, out_dim: int, num_layers: int, dropout,
        ...
        print("Running: Model_EdgeCycle_Aut_double_0th_batch_2\n\n")
        super().__init__()
