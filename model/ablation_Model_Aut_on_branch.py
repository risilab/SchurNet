...
class Edge_branched_cycle(torch.nn.Module):
    ...
    def __init__(self, rep_dim, num_channels,use_branched_5_cycles,cycle_sizes) -> None:
        super().__init__()
        self.cycle_mlp = get_mlp((num_channels * 3 + 3) * rep_dim,rep_dim * _inner_mlp_mult,rep_dim,3) # one more layer mlp
    ...
    def forward(self,edge_rep,cycle_rep,G,data):
        ...
        br_cycle2cycles = [p.batched_subgraphlayer1b.gather_from_ptensors(edge2br_cycle_aut,G,cycle,min_overlaps=size) 
                           for cycle,size in zip(self.cycles,self.cycle_sizes)]  #min_overlaps=3 to make sure only cycle to itself]
        br_cycle2cycle = p.batched_subgraphlayer1b.cat(*br_cycle2cycles) #nc * num_channels * 2
        ...
        cycle_out = self.cycle_mlp(torch.cat([cycle_rep.torch(),edge2cycle_aut.torch(),
                                              edge2cycle_linmap.torch(),br_cycle2cycle.torch()],dim=-1)) #nc * (num_channels * 3 + 3) -> nc
...
class ablation_Model_Aut_on_branch(torch.nn.Module):
    def __init__(self, rep_dim: int, dense_dim: int, out_dim: int, num_layers: int, dropout,
        ...
        print("Running: ablation_Model_Aut_on_branch\n\n")
        super().__init__()
