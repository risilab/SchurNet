...
class Edge_branched_cycle(torch.nn.Module):
    ...
    def __init__(self, rep_dim, num_channels,use_branched_5_cycles,cycle_sizes) -> None:
        super().__init__()
        self.cycle_mlp = get_mlp((4 + 3) * rep_dim,rep_dim * _inner_mlp_mult,rep_dim,3) # one more layer mlp
    ...
    def forward(self,edge_rep,cycle_rep,G,data):
        ...
        edge2br_cycles_linmap = [p.batched_subgraphlayer1b.gather_from_ptensors(edge2br_cycle,G,br_cycle,min_overlaps=size) 
        ...
        br_cycle2cycles = [p.batched_subgraphlayer1b.gather_from_ptensors(edge2br_cycle_linmap,G,cycle,min_overlaps=size) 
                           for cycle,size in zip(self.cycles,self.cycle_sizes)]  #min_overlaps=3 to make sure only cycle to itself]
        br_cycle2cycle = p.batched_subgraphlayer1b.cat(*br_cycle2cycles) #nc * 4
        
        edge2cycles = [p.batched_subgraphlayer1b.gather_from_ptensors(edge_rep,G,cycle,min_overlaps=1) for cycle in self.cycles]
        ...
        cycle_out = self.cycle_mlp(torch.cat([cycle_rep.torch(),
                                              edge2cycle_linmap.torch(),br_cycle2cycle.torch()],dim=-1)) #nc * (4 + 3) -> nc
...
class ConvLayer(torch.nn.Module):
    def __init__(self, rep_dim: int,num_channels, dropout,cycle_sizes,use_branched_5_cycles, gather_overlap) -> None:
        ...
        self.edge_br_cycle = Edge_branched_cycle(rep_dim, num_channels, use_branched_5_cycles, cycle_sizes)
    ...
    def forward(self, node_rep, edge_rep,cycle_rep,G, data):
        ...
        edge_out2, cycle_out = self.edge_br_cycle(edge_rep,cycle_rep,G,data)
...
class ablation_Model_Linmap_on_branch(torch.nn.Module):
    def __init__(self, rep_dim: int, dense_dim: int, out_dim: int, num_layers: int, dropout,
                 readout, ds_name,num_channels,dataset,cycle_sizes,use_branched_5_cycles,gather_overlap = 1,device = 'cuda') -> None:
        print("Running: ablation_Model_Linmap_on_branch\n\n")
        super().__init__()
