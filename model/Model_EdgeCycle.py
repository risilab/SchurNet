...
class Edge_Cycle(torch.nn.Module):
    ...
    def __init__(self, hidden_dim) -> None:
        ...
        # self.autobahn_cycle_5 = p.Autobahn(2 * hidden_dim,hidden_dim,self.cycle5)
        # self.autobahn_cycle_6 = p.Autobahn(2 * hidden_dim,hidden_dim,self.cycle6)
        # self.autobahn_cycle_5 = p.Linear(4 * hidden_dim,hidden_dim)
        # self.autobahn_cycle_6 = p.Linear(4 * hidden_dim,hidden_dim)
    ...
    def forward(self,edge_rep: ptens1_type,cycle_rep: ptens1_type,G):
        ...
        # print("Before autobahn")
        # print(f"edge2cycle5: \n {edge2cycle5}")

        # edge2cycle5 = self.autobahn_cycle_5(edge2cycle5)
        # edge2cycle6 = self.autobahn_cycle_6(edge2cycle6) #nc * 4 -> nc

        # print("after autobahn")

        cycle_in = p.subgraphlayer1b.cat(edge2cycle5,edge2cycle6) 
        # cycle_in = p.ptensors1b.linmaps(cycle_in) #nc * 6
        cycle_new = p.subgraphlayer1b.cat_channels(cycle_in,cycle_rep) #nc * 5; nc * 2

        cycle2edge = p.subgraphlayer1b.gather_from_ptensors(cycle_new,G,self.edge) #nc * 10; nc * 4
...
class Model_EdgeCycle(torch.nn.Module):
